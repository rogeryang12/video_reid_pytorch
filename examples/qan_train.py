...
def main():
    ...
    # parser.add_argument('--h5_file', default='data/train.h5')
    ...
    model = QAN(args.emb_dim, len(dataset.pids), args.cnn, ckpt=args.ckpt)
    ...
    def cal_loss(data, model):
        ...
        images, labels = images.view(-1, i, c, h, w).to(device), labels.view(-1, i).to(device)
        embs, v_embs, logits = model(images)
        loss1, prec = celoss(logits, labels.view(-1))
        loss2, top1 = triloss(v_embs, labels[:, 0].contiguous())
        loss = loss1 + loss2
